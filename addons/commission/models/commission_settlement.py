# gérer les règlements de commissions et les lignes de règlement respectivement
...
class CommissionSettlement(models.Model):
    ...
    total = fields.Float(compute="_compute_total", readonly=True,
                         store=True)  # Champ calculé qui représente le montant total du règlement en fonction des lignes de règlement associées
    ...
    agent_id = fields.Many2one(
        ...
        domain="[('agent', '=', True)]",
        required=True,
    )  # Champ pour l'identifiant de l'agent associé au règlement.
    ...
    # Relation One2many vers les lignes de règlement associées à ce règlement
    ...
    # Champs pour la devise et l'entreprise associées au règlement
    ...
    # currency_id et company_id : Champs pour la devise et l'entreprise associées au règlement
    ...
    def action_cancel(self):
        self.write({"state": "cancel"})  # Méthode pour annuler le règlement en mettant à jour son état
...
class SettlementLine(models.Model):
    ...
    def _compute_settled_amount(self):
        """
        """
# @api.depends pour recalculer automatiquement les champs calculés en fonction des valeurs mises à jour dans d'autres champs
