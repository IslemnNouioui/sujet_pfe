...
class CommissionMixin(models.AbstractModel):
    ...
    agent_ids = fields.One2many(
        ...
        store=True,
        copy=True,
    )  # agent_ids : Relation One2many vers la classe commission.line.mixin
    product_id = fields.Many2one(comodel_name="product.product",
                                 string="Product")  # Relation Many2one vers la classe product.product, représentant le produit associé à l'objet
    commission_free = fields.Boolean(
        string="Comm. free",
        compute="_compute_commission_free",
        store=True,
        readonly=True,
    )  # Champ booléen indiquant si la commission est libre pour cet objet
    ...
    def _prepare_agent_vals(self, agent):
        return {"agent_id": agent.id,
                "commission_id": agent.commission_id.id}  # Prépare les valeurs des agents pour être utilisées dans les champs One2many

    def _prepare_agents_vals_partner(self, partner, settlement_type=None):
        """Utilitaire pour obtenir un dictionnaire de création d'agents pour un partenaire donné"""
    ...
    def button_edit_agents(self):  # Ouvre une vue pour éditer les agents
        self.ensure_one()
...
class CommissionLineMixin(models.AbstractModel):
    ...
    object_id = fields.Many2one(
        ...
        copy=False,
        string="Parent",
    ) # Relation Many2one vers la classe commission.mixin, représentant l'objet parent
    agent_id = fields.Many2one(
        comodel_name="res.partner",
        domain="[('agent', '=', True)]",
        ondelete="restrict",
        required=True,
    ) # Relation Many2one vers la classe res.partner, représentant l'agent de commission.
    commission_id = fields.Many2one(
        ...
        readonly=False,
        copy=True,
    ) #Relation Many2one vers la classe commission, représentant la commission associée à l'agent.
    ...
    currency_id = fields.Many2one(comodel_name="res.currency") #Devise de la commission.
